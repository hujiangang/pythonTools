...
class WatermarkRemoverASR:
    ...
    def __init__(self, input_path: str, output_dir: str):
        ...
        if not self.input_path.exists():
            raise FileNotFoundError(f"输入视频不存在：{self.input_path}")
    ...
    def _unique_path(path: Path) -> Path:
        """Return a writable-looking path without overwriting an existing file."""
        if not path.exists():
            return path
    ...
    def remove_watermark(self) -> Optional[Path]:
        """去除水印"""
        print("\n🔧 去除水印...")
        clean_video = self._unique_path(self.output_dir / f"{self.input_path.stem}_clean.mp4")
        ...
        try:
            ...
            print(f"  ⚠️ 水印去除失败：{e}")
            print("  使用原始视频继续...")
            shutil.copy2(self.input_path, clean_video)
            return clean_video
        return None
...
"""
    ...
    def burn_subtitle(self, video_path: Path, subtitle_path: Path) -> Optional[Path]:
        """烧录字幕"""
        print("\n🎬 烧录字幕...")
        output_video = self._unique_path(self.output_dir / f"{video_path.stem}_sub.mp4")

        # FFmpeg 的滤镜参数在 Windows 下需要转义盘符和反斜杠。
        subtitle_filter_path = str(subtitle_path.resolve()).replace('\\', '/').replace(':', r'\:')
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='水印去除 v3.0 - 增强版')
    ...
    try:
        processor = WatermarkRemoverASR(args.input, args.output)
